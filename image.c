...
void read_img(const char* filename, Image* img) {
    ...
    if (img->row_pointers) exit(1); // se o ponteiro para colunas n�o entiver vazio
...
void normalize_img(Image* img, Point* start, Point* end) {
    for (int y = 0; y < img->height; y++) {
        ...
        for (int x = 0; x < img->width; x++) {
            ...
            else if ((px[2] > 100 && px[1] < 80 && px[0] < 80)) {
                px[0] = 0; px[1] = 255; px[2] = 0;
                end->x = x;
                end->y = y;
                img->matrix[x][y] = 0;
